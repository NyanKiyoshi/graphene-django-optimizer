...
class QueryOptimizer(object):
    ...
    def _optimize_gql_selections(self, field_type, field_ast):
        ...
        possible_types = self._get_possible_types(graphql_type)  # Note: Part where it gets selection
        for selection in selection_set.selections:
            print('Optimizing: ' + repr(selection.name))
...
class QueryOptimizerStore():
    ...
    def optimize_queryset(self, queryset):
        print(
            f'Prefetching `{self.prefetch_list}` for `{queryset.model.verbose_name}`')
